 ...
 * </pre>
 *
 * <h1>Implementation details</h1> <h2>Representation of the rotations</h2>
 * <p>
 * Storing all rotations as {@code String}s or as a matrix of {@code Character}s
 * would be a total waste of space since the rotations of the source block contain
 * the exact same characters. Therefore, the source block is stored only once (as the
 * original {@code String}), and each rotation is represented by an integer defined
 * as follows: the rotation i is obtained by rotating the source block i times to the
 * left (or i characters to the left). For instance, "lloHe" is the rotation 2 of
 * "Hello", and the source block is always the rotation 0.
 * </p>
 * <h2>Representation of the matrix</h2>
 * <p>
 * The matrix can then be represented by an array of integer, whose indices are the
 * rows of the matrix, and whose values are the rotation number (that we have just
 * defined) corresponding to each row. For example, here is the matrix that we have
 * seen for the source "Brave" (before sorting):
 * </p>
 *
 * <pre>
 ...
 * </pre>
 * <p>
 * This matrix would be represented by the array {@code [0, 1, 2, 3, 4]}. Indeed, for
 * all i, the rotation i is the row i of the matrix. And after being
