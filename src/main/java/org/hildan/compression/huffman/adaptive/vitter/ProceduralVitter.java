...
/**
 * The original Pascal code of Vitter's paper, directly translated in Java, without object paradigm
 * adaptation.
 * <p>
 ...
 * <dd>alphabet size</dd>
 * <dt>a<sub>j</sub></dt>
 * <dd>j<sup>th</sup> letter in the alphabet. Here we'll directly use the {@code char a}<sub>j</sub>
 * instead of the {@code int j} itself.</dd>
 * <dt>t</dt>
 ...
 * </dl>
 * <p>
 * In the paper, arrays are indexed starting at 1. 0 is used to denote {@code null}. It is
 * reproduced here, the index 0 kept unused.
 * </p>
 * <p>
 * Other definitions:
 * </p>
...
public class ProceduralVitter {
    ...
    /**
     * {@code parent[b]} = the parent node of the leader of block b, if it exists; and 0 otherwise
     */
    private final int[] parent;

    /**
     * {@code parity[b]} = 0 if the leader of block b is a left child or the root of the Huffman
     * tree; and 1 otherwise
     */
    private final int[] parity;

    /**
     * {@code rtChild[b]} = the right child of the leader of the block {@code b} (if {@code b} is a
     * block of internal nodes).
     */
     ...
     */
    /**
     * First block in the available-block list, if the list is not empty; and 0 otherwise.
     */
    private int availBlock;

    /**
     * {@code stack[i]} = i<sup>th</sup>-to-last bit of encoding of the current letter being
     * processed
     */
    ...
    private void update(char c) {
        ...
        while (q > 0) {
            /*
             * At this point, q is the first node in its block. Increment q’s weight by 1 and slide
             * q if necessary over the next block to maintain the invariant. Then set q to the node
    ...
    private void findNode(char c) {
        ...
        if (q <= M) {
            ...
            if (M > 0) {
                /*
                 * Split the O-node into an internal node with two children. The new O-node is node
                 * M; the old O-node is node M + 1; the new parent of nodes M and M + 1 is node M +
                 * n
                 */
                block[M] = bq;
                last[bq] = M;
