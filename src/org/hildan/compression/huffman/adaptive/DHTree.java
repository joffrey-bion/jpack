package org.hildan.compression.huffman.adaptive;
...
public class DHTree {
	...
	private void update() {
		final DHTree max = getMaxInBlock();
	...
	private DHTree getLeaf(Character c) {
		if (isLeaf()) {
			...
			final DHTree node = zero.getLeaf(c);
	...
	private void swapWith(DHTree tree) {
		...
		final DHTree treeParent = tree.parent;
		tree.parent = parent;
		parent = treeParent;
		final int treeNumber = tree.number;
		tree.number = number;
		number = treeNumber;
	}
